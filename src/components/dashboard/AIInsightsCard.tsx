...
export function AIInsightsCard() {
    ...
    const generateInsights = async (isManual = false) => {
        ...
        try {
            ...
            // Apply actual task updates generated by the AI
            if (result.suggestedTaskUpdates && result.suggestedTaskUpdates.length > 0) {
                const { updateTask } = useTaskStore.getState();
                for (const update of result.suggestedTaskUpdates) {
                    if (update.priority) {
                        try {
                            await updateTask(update.taskId, { priority: update.priority as any });
                        } catch (e) {
                            console.error('Failed to apply AI task update', e);
                        }
                    }
                }
            }
